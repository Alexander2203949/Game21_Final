...
void push_in_vector(vector<int> & vec, int num); //помещает сгенерированное число в вектор
...
int summ_player1; //сумма 1 игрока
int summ_player2; // сумма 2 игрока
...
int main()
{
    srand((unsigned) time(NULL));// момогает рандому задействовать системное время
    game_player(1);
    return 0;
}
